			...
			case RC_DRAW_SURFS:
				...
				for( i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs; i++, drawSurf++ ) {
					...
					if( sortedIndex >= newShader ) {
						sortedIndex++;
#ifdef CMOD_FRCL_ENTITYNUM_FIX
						drawSurf->sort = (sortedIndex << QSORT_SHADERNUM_SHIFT) | (entityNum << QSORT_REFENTITYNUM_SHIFT) |
								( fogNum << QSORT_FOGNUM_SHIFT ) | ( (int)pshadowMap << QSORT_PSHADOW_SHIFT) | (int)dlightMap;
#else
...
shader_t *R_FindShader( const char *name, int lightmapIndex, qboolean mipRawImage ) {
	...
	if ( tr.new_filesystem ) {
		int flags = LOOKUPFLAG_ENABLE_MTR;
		if ( r_ext_compressed_textures->integer ) {
			flags |= LOOKUPFLAG_ENABLE_DDS;
		}
		fsShader = ri.FS_ShaderLookup( strippedName, flags, qfalse );
