...
#ifdef CMOD_SETTINGS
DEF_PUBLIC( fileHandle_t FS_OpenGlobalSettingsFileWrite( const char *filename ) )
#endif
...
DEF_PUBLIC( fileHandle_t FS_Basedir_FOpenFileAppend( xdg_home_type_t xdgType, const char *filename ) )
#endif
...
#ifdef CMOD_RECORD
DEF_PUBLIC( void FS_BaseDir_Rename_HomeData( const char *from, const char *to, qboolean safe ) )
#endif
...
DEF_PUBLIC( void *cMod_FS_ObtainDownloadMap( void ) )
DEF_PUBLIC( void cMod_FS_FreeDownloadMap( void *dlmap ) )
DEF_PUBLIC( fileHandle_t cMod_FS_OpenDownloadPak( const char *path, unsigned int *size_out, void *dlmap ) )
#endif
