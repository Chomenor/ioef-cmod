...
#ifdef ELITEFORCE
		if ( FS_idPak( buffer, BASEGAME, FS_NODOWNLOAD_PAKS ) ) {
#else
...
void FS_GenerateReferenceLists( void ) {
	...
	for ( i = 0; i < download_list.entry_count; ++i ) {
		...
		if ( !*client_mod_dir ) {
			client_mod_dir = Cvar_VariableString( "fs_game" );
		}
...
void *cMod_FS_ObtainDownloadMap( void ) {
	...
	while ( ( old_entry = (download_map_entry_t *)FS_Hashtable_Next( &it ) ) ) {
		...
		*new_entry = *old_entry;
		...
		FS_Hashtable_Insert( new_map, (fs_hashtable_entry_t *)new_entry, FSC_StringHash( new_entry->name, NULL ) );
	}
...
/*
=================
cMod_FS_FreeDownloadMap
=================
*/
void cMod_FS_FreeDownloadMap( void *dlmap ) {
	if ( !dlmap ) {
		return;
	}
	FS_DLMap_Free( (fs_download_map_t *)dlmap );
}

/*
=================
cMod_FS_OpenDownloadPak
=================
*/
fileHandle_t cMod_FS_OpenDownloadPak( const char *path, unsigned int *size_out, void *dlmap ) {
	if ( dlmap ) {
		return FS_DLMap_OpenPak( (fs_download_map_t *)dlmap, path, size_out );
	}
	return 0;
}
#endif
