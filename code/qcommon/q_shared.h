...
//  #define PROTOCOL_HANDLER		"foobar"
  ...
  #define HOMEPATH_NAME_MAC			"STVEF"
...
#define CVAR_TRUSTED_CREATED (1<<14)			// current value is not from a restricted source such as server or untrusted VM
#define CVAR_ENGINE_REGISTERED (1<<15)			// indicates that there has been an engine Cvar_Get call on this cvar
#define CVAR_RESTRICTED_MODIFIABLE (1<<16)		// allow temporary modifications to this var from untrusted VMs
#define CVAR_VM_PERSISTENT (1<<17)				// allow values set by trusted VMs to persist and be written to settings file
#define CVAR_OVERRIDE_ENGINE_DEFAULT (1<<18)	// don't update default value on future calls to Cvar_Get
#define CVAR_IMPORT_ALLOWED (1<<19)				// allow modifying this var in import/protected autoexec mode
#define CVAR_DEFER_TO_VM_DEFAULT (1<<20)		// allow vm default to override system default
#define CVAR_NOARCHIVE (1<<21)					// forcibly ignore CVAR_ARCHIVE
#define CVAR_NUMERIC (1<<22)					// perform numeric comparison with default when deciding to write the cvar to config file
#define CVAR_IGNORE_VM_CHEAT_PROTECT (1<<23)	// ignore CVAR_CHEAT flag
#ifdef CMOD_SAFE_AUTOEXEC
#define CVAR_RESTRICTED_AUTOEXEC_CREATED (1<<24)	// current value is set by the autoexec safe import procedure
#endif
#endif
...
struct cvar_s {
	...
	// Current values (generated by Cvar_Finalize)
	char *string;
	float value;
	int integer;
	...
	qboolean modified;			// set each time the cvar is changed (can be directly cleared from anywhere)
