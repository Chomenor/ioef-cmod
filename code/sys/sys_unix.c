...
#ifdef ELITEFORCE
#ifdef __APPLE__
#define HOMEPATH_NAME HOMEPATH_NAME_MAC
#else
...
==================
*/
static char *Sys_HomeCachePath(void)
{
	static char homeCachePath[ MAX_OSPATH ] = { 0 };
	char *p;

	if( !*homeCachePath )
	{
		if( ( p = getenv( "XDG_CACHE_HOME" ) ) != NULL && *p != '\0' )
			Com_sprintf(homeCachePath, sizeof(homeCachePath), "%s%c", p, PATH_SEP);
		else if( ( p = getenv( "HOME" ) ) != NULL && *p != '\0' )
			Com_sprintf(homeCachePath, sizeof(homeCachePath), "%s%c.cache%c", p, PATH_SEP, PATH_SEP);

		if( *homeCachePath )
		{
			if( com_homepath && com_homepath->string[0] )
				Q_strcat(homeCachePath, sizeof(homeCachePath), com_homepath->string);
			else
				Q_strcat(homeCachePath, sizeof(homeCachePath), HOMEPATH_NAME);
		}
	}
...
/*
==================
Sys_DefaultNonXdgHomepath
==================
*/
char *Sys_DefaultNonXdgHomepath(void) {
	return Sys_LegacyHomePath();
}

/*
==================
Sys_DefaultXdgHomepath
==================
*/
char *Sys_DefaultXdgHomepath(xdg_home_type_t xdgType) {
	switch (xdgType) {
		case XDG_DATA:
...
void Sys_ErrorDialog( const char *error )
{
	...
	FS_GeneratePathWritedir( XDG_DATA, "crashlog.txt", NULL, 0, 0, ospath, sizeof( ospath ) );
#else
