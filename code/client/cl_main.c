...
#ifdef USE_HTTP
	if ( clc.state > CA_DISCONNECTED && !clc.httpReconnecting ) {
#else
...
#define RENDERER_PREFIX "renderer_"
	...
	Com_sprintf(dllName, sizeof(dllName), RENDERER_PREFIX "%s" DLL_EXT, cl_renderer->string);
#else
...
#ifdef ELITEFORCE
		Com_sprintf(dllName, sizeof(dllName), RENDERER_PREFIX "opengl1" DLL_EXT);
#else
